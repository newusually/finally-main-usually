...
func generateAnimalProblem() (string, float64) {
	...
	// 生成题目参数（网页1动态规划思路）
	heads := rand.Intn(30) + 10
	totalLegs := heads*2 + rand.Intn(20)*2
...
// ================== 主程序 ==================
func main() {
	file, err := os.Create("math_questions.csv")
	if err != nil {
	...
	writer.Write([]string{"题号", "类型", "题目", "答案"})
	...
	fmt.Println("已生成", questionCount, "道题目到math_questions.csv")
}
